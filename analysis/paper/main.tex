...
\hypersetup{
  ...
  urlcolor=cyan,
  citecolor=red,
  pdftitle={Assumption-Bounded Analytic Screening for IFRS-16 Covenant Analysis},
...
Assumption-Bounded Analytic Screening for IFRS-16 Covenant Analysis}
\author{
Aniket Bhardwaj\\
...
\begin{abstract}
We optimize LBO covenants under IFRS-16 and frozen-GAAP conventions. A Bayesian hierarchical calibration supplies data-informed priors; closed-form headroom approximations provide assumption-bounded diagnostics for rapid screening and comparison with full simulation. The repository releases a reproducible IFRS-16 LBO benchmark, code, and scripts for regenerating reported artifacts.
\end{abstract}
...
We address this gap by developing a framework that treats covenant levels as optimization variables under Bayesian uncertainty across both accounting conventions. Our approach replaces ad hoc covenant assumptions with data-informed priors using bounded-support transformations, incorporates explicit IFRS-16 lease amortization schedules, and provides assumption-bounded diagnostics for rapid screening.

\subsection{Contributions}

\begin{itemize}\setlength{\itemsep}{0pt}
\item \textbf{Theory:} Assumption-bounded diagnostics for leverage and ICR approximations; a conservative screening rule when analytic headroom exceeds a computable $\varepsilon_{\max}$.
...
\item \textbf{Resource:} An open, reproducible IFRS-16 LBO benchmark with scripts that regenerate all figures and tables in one command.
\end{itemize}

The framework reports evaluation metrics from the benchmark artifacts at runtime and compares analytic screening against full simulation under both covenant conventions.
...
We optimize covenant packages $\mathcal{C} = (c^{\text{lev}}, c^{\text{icr}})$ under safety-constrained feasibility where $c^{\text{lev}}$ is the maximum leverage ratio threshold and $c^{\text{icr}}$ is the minimum interest coverage ratio threshold. The safety-constrained optimization maximizes expected IRR subject to assumption-bounded screening constraints:
\begin{align}
...
\section{Assumption-Bounded Approximation Diagnostics}
...
\subsection{Approximation Diagnostics}
...
\begin{proposition}[Assumption-Bounded Screening Result]\label{prop:screening}
...
If $h_{\text{analytic}}(t)>\epsilon_{\max}$ for all $t$, the true covenant package is expected to remain feasible within the stated approximation assumptions.
\end{proposition}
...
We calibrate to hotel operator disclosures, defining 5 tracked archetypes across regions. Targets: revenues $\$0.8$B--$\$1.7$B, EBITDA margins 20\%--24\%, lease/EBITDA 2.8x--3.4x, and asset-light/heavy mixes.
...
\textbf{Task 3: Optimal Covenant Design} (expected IRR subject to assumption-bounded screening).
...
\textbf{Sample Size and Clustering:} We evaluate on 5 tracked hotel archetypes with 40 sampled scenarios per archetype (200 total scenarios), each simulated over 5 annual steps. Operators vary by region and balance-sheet intensity.
...
\textbf{Baseline Comparison:} We report the benchmark metrics directly from the regenerated artifact so the manuscript remains synchronized with the code and data.

\textbf{Reproducibility:} All reported results are generated from the repository at a fixed seed. Environment specification in \texttt{environment.yml} ensures exact package versions.
...
We advance safety-constrained covenant design under IFRS-16 via Bayesian calibration, assumption-bounded analytic screening, and a standardized benchmark. Reported metrics are regenerated from the repository artifacts so the manuscript stays synchronized with the code and data.
