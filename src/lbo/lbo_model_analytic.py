...
class AnalyticAssumptions:
    ...
    lease_treatment: str = "steady_state"  # "steady_state" or "run_off"
    lease_principal_rate: float = 0.12
    lease_additions_rate: float = 0.01
...
class AnalyticResults:
    ...
    lease_treatment: str = "steady_state"
...
class AnalyticLBOModel:
    ...
    def solve_paths(self) -> AnalyticResults:
        ...
        r_debt = a.senior_fraction * a.senior_rate + (1 - a.senior_fraction) * a.mezz_rate
        ...
        self.results = AnalyticResults(
            ...
            icr_ratio=icr_ratio,
            lease_treatment=a.lease_treatment,
        )
    ...
    def compute_elasticities(self, epsilon: float = 0.01) -> Dict[str, np.ndarray]:
        ...
        for param_name, attr_name in params:
            ...
            leverage_elasticity = (
                (shocked_results.leverage_ratio - base_leverage) / base_leverage
            ) / epsilon
            icr_elasticity = ((shocked_results.icr_ratio - base_icr) / base_icr) / epsilon
    ...
    def validate_against_simulation(
        ...
        if np.sum(finite_mask) > 0:
            icr_rel_error = np.abs(analytic_icr[finite_mask] - sim_icr[finite_mask]) / np.abs(
                sim_icr[finite_mask]
            )
        ...
        for i in range(len(param_names)):
            for j in range(len(years_subset)):
                ax.text(
                    ...
                    ha="center",
                    va="center",
                    color="black" if abs(elasticity_matrix[i, j]) < 0.5 else "white",
                )
        ...
        for i in range(len(param_names)):
            for j in range(len(years_subset)):
                ax.text(
                    ...
                    ha="center",
                    va="center",
                    color="black" if abs(elasticity_matrix_capped[i, j]) < 1 else "white",
                )
...
def main():
    ...
    for param in ["growth_rate", "cash_sweep", "senior_rate"]:
        ...
        if lev_key in elasticities:
            print(
                f"  {param}: ∂Leverage = {elasticities[lev_key][1]:.3f}, ∂ICR = {elasticities[icr_key][1]:.3f}"
            )
    ...
    if args.validate:
        ...
        mock_sim = {
            ...
            + np.random.normal(0, 0.05, len(results.leverage_ratio)),
