...
def ratios_ifrs16(row: pd.Series) -> Tuple[float, float]:
    ...
    total_interest = (
        row.fin_rate * (row.debt_senior + row.debt_mezz) + row.lease_rate * row.lease_liability
    )
...
def covenant_headroom(leverage: float, icr: float, c_lev: float, c_icr: float) -> Dict[str, float]:
